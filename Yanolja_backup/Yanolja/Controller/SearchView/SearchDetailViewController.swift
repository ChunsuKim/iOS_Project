...
extension SearchDetailViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if singleTon.saveSearchList.count == 0 {
            return 1
        } else {
        return singleTon.saveSearchList.count
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "default", for: indexPath)
            ...
            let data = singleTon.saveSearchList[indexPath.row]
            
//            cell.put(mainImage: data.mainImage, stay: data.stay, averageGrade: data.averageGrade, totalComments: data.totalComments, directions: data.directions, daysPrice: data.daysPrice, rentableTime: data.hoursAvailable, stableTime: data.hoursAvailable)
            return cell
        }
    }
