//
//  HomeThemeSecondCollectionViewMenuBar.swift
//  Yanolja
//
...
// MARK: - Protocol
protocol SecondMenuBarDelegate {
...
class HomeThemeSecondCollectionViewMenuBar: UIView {
    ...
    let menuSecondCollectionView: UICollectionView = {
    ...
    private func configureCollectionView() {
        menuSecondCollectionView.delegate = self
        menuSecondCollectionView.dataSource = self
        
        
        menuSecondCollectionView.register(HomeThemeMenuCollectionCell.self, forCellWithReuseIdentifier: HomeThemeMenuCollectionCell.identifier)
    }
    ...
    private func setSelectedMenuBar() {
        menuSecondCollectionView.selectItem(at: IndexPath(item: 0, section: 0), animated: true, scrollPosition: .centeredHorizontally)
    }
    ...
    private func configureConstraints() {
        addSubview(menuSecondCollectionView)
        addSubview(lineViewTop)
        addSubview(lineViewBottom)
        addSubview(indicatorBar)
        
        menuSecondCollectionView.topAnchor.constraint(equalTo: topAnchor).isActive = true
        menuSecondCollectionView.leadingAnchor.constraint(equalTo: leadingAnchor).isActive = true
        menuSecondCollectionView.trailingAnchor.constraint(equalTo: trailingAnchor).isActive = true
        menuSecondCollectionView.bottomAnchor.constraint(equalTo: bottomAnchor).isActive = true
        ...
        lineViewTop.leadingAnchor.constraint(equalTo: menuSecondCollectionView.leadingAnchor).isActive = true
        lineViewTop.trailingAnchor.constraint(equalTo: menuSecondCollectionView.trailingAnchor).isActive = true
        ...
        indicatorBarWidthConst = indicatorBar.widthAnchor.constraint(equalTo: menuSecondCollectionView.widthAnchor, multiplier: 0.001)
    ...
    // MARK: - Global Method
    func reloadSecondMenuCollectionView() {
        isState.toggle()
        menuSecondCollectionView.reloadData()
        setSelectedMenuBar()
    }
...
extension HomeThemeSecondCollectionViewMenuBar: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isState {
            cell.menuLabel.text = menubarDataThirdTheme[indexPath.row]
            homeThemeMenuCollectionView = cell
        } else {
            cell.menuLabel.text = menubarDataForthTheme[indexPath.row]
            homeThemeMenuCollectionView = cell
        }
...
extension HomeThemeSecondCollectionViewMenuBar: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print("didSelect")
        delegate?.secondMenuBarDidSelected(indexPath)
...
extension HomeThemeSecondCollectionViewMenuBar: UICollectionViewDelegateFlowLayout {
